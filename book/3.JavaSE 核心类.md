...
Collection 接口是集合的顶级父类，其下有两个子类：List (线性表)和 Set。List 可重复集，有序的。Set 不可重复集，大部分是无序的。
...
泛型可以在使用某个类的时候向传递参数一样来向这类中某些参数传递一个类型。

动态的决定这个被创建出来的类的实例的方法(参数和返回值)，属性的类型，从而提高程序的灵活性。
